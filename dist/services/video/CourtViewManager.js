...
exports.CourtViewManager = void 0;
class CourtViewManager {
    constructor(courtCoverageService, initialCamera = 'camera_a', outputConfig) {
        this.lastSwitchTime = 0;
        this.switchHistory = [];
        // Default switching parameters
        this.MIN_SWITCH_INTERVAL = 1000; // 1 second minimum between switches
        this.CONFIDENCE_THRESHOLD = 0.7; // Minimum confidence for switching
        this.POSITION_HYSTERESIS = 1.0; // 1 meter hysteresis zone
     ...
     */
    selectOptimalCamera(ballPosition, ballDetections, activeTracks) {
        const currentTime = Date.now();
        // Get recommended camera from court coverage service
        const recommendedCamera = this.courtCoverageService.getOptimalCamera(ballPosition.x, ballPosition.y);
        ...
        const shouldSwitch = this.shouldSwitchCamera(recommendedCamera, ballPosition, ballDetections, currentTime);
        const confidence = this.calculateSwitchConfidence(ballPosition, ballDetections, recommendedCamera);
        let transitionInfo;
     ...
     */
    executeCameraSwitch(newCameraId, ballPosition, reason) {
        const switchEvent = {
     ...
     */
    shouldSwitchCamera(recommendedCamera, ballPosition, detections, currentTime) {
     ...
     */
    calculateSwitchConfidence(ballPosition, detections, targetCamera) {
     ...
     */
    determineSwitchReason(ballPosition, detections) {
     ...
     */
    buildSelectionReason(ballPosition, recommendedCamera, shouldSwitch) {
        const netY = 10;
     ...
     */
    calculateDistanceFromZoneBoundary(ballPosition) {
...
exports.CourtViewManager = CourtViewManager;
//# sourceMappingURL=CourtViewManager.js.map