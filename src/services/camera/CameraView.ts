...
export class CameraView implements ICameraView {
  public id: string;
  public name: string;
  public mountPosition: CameraMountPosition;
   ...
   */
  public calculateVisibleArea(): Rectangle {
   ...
   */
  public calculateBlindArea(): Rectangle {
    const { width, length } = CameraView.COURT_DIMENSIONS;
   ...
   */
  private calculateNearVisibleDistance(cameraHeight: number): number {
    ...
    const minVisibleDistance = Math.max(
      3.0, // Minimum 3 meters from camera
   ...
   */
  public canSeePoint(courtX: number, courtY: number): boolean {
    const visibleArea = this.calculateVisibleArea();
    
    return (
      courtX >= visibleArea.topLeft.x &&
   ...
   */
  public getOptimalZone(): Rectangle {
   ...
   */
  public pixelToCourtCoordinates(pixelX: number, pixelY: number): Point2D | null {
   ...
   */
  public courtToPixelCoordinates(courtX: number, courtY: number): Point2D | null {
    if (!this.calibration?.homographyMatrix) {
      return null;
    }
   ...
   */
  private calculateMatrixInverse(matrix: number[][], det: number): number[][] {
    const [[a, b, c], [d, e, f], [g, h, i]] = matrix;
