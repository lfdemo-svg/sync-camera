import { CourtViewManager } from './CourtViewManager';
...
export class VideoOutputService {
  private courtViewManager: CourtViewManager;
  ...
  private outputBuffer: VideoOutput[] = [];

  // Default configuration optimized for Padel court recording
  private static readonly DEFAULT_CONFIG: OutputConfiguration = {
    mode: 'follow_ball',
  ...
  constructor(
    ...
    positionTracker: PositionTrackingService,
    config?: Partial<OutputConfiguration>
  ) {
    ...
    this.courtViewManager = new CourtViewManager(
      courtCoverageService,
      'camera_a',
      outputConfig
    );
    
    this.videoProcessor = new VideoProcessor(outputConfig);
   ...
   */
  public startRecording(): RecordingSession {
    if (this.isRecording) {
    ...
    this.currentSession = {
      sessionId,
      startTime: new Date(),
      outputConfig: this.videoProcessor.getPerformanceStats() as any, // Simplified
      totalFrames: 0,
      outputFrames: [],
      switchHistory: [],
      ballTrajectory: [],
   ...
   */
  public processFrames(frames: Map<string, VideoFrame>): VideoOutput | null {
    if (!this.isRecording || !this.currentSession) {
      return null;
    }
    ...
    try {
      ...
      const cameraSelection = this.courtViewManager.selectOptimalCamera(
        ballDetection?.courtPosition || { x: 5, y: 10 },
        detectionResult.detections,
        detectionResult.activeTracks
      );

      // Execute camera switch if needed
      let switchEvent: CameraSwitchEvent | undefined;
      if (cameraSelection.shouldSwitch && cameraSelection.transitionInfo) {
        switchEvent = this.courtViewManager.executeCameraSwitch(
          cameraSelection.selectedCamera,
          ballDetection?.courtPosition || { x: 5, y: 10 },
          cameraSelection.transitionInfo.reason
        );
      ...
      const videoOutput = this.videoProcessor.processFrames(
        frames,
        cameraSelection.selectedCamera,
        ballDetection,
        cameraSelection.transitionInfo
      );
  ...
  public updateConfiguration(newConfig: Partial<OutputConfiguration>): void {
    ...
    if (this.currentSession) {
      this.currentSession.outputConfig = { ...this.currentSession.outputConfig, ...newConfig } as any;
    }
   ...
   */
  public generateSessionReport(session?: RecordingSession): string {
   ...
   */
  private validateSystemReadiness(): void {
   ...
   */
  private calculateFinalStatistics(trackingStats: any): SessionStatistics {
    if (!this.currentSession) {
    ...
    const cameraUsage: { [key: string]: any } = {};
   ...
   */
  private getSessionDuration(session: RecordingSession): number {
