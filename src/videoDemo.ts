import { VideoOutputService } from './services/video/VideoOutputService';
...
async function demoVideoOutputSystem() {
  ...
  const syncService = new SynchronizationService(['camera_a', 'camera_b']);
  ...
  const videoOutputService = new VideoOutputService(
    courtService,
    syncService,
    positionTracker,
    {
  ...
  for (const mode of outputModes) {
    ...
    videoOutputService.updateConfiguration({ mode: mode as any });
    ...
    const testFrames = new Map([
      ['camera_a', {
        ...
        metadata: { width: 1920, height: 1080, fps: 60, capturedAt: testTimestamp, receivedAt: testTimestamp, format: 'rgb' as const }
      }],
      ['camera_b', {
        ...
        frameNumber: 1000,
        data: Buffer.alloc(1920 * 1080 * 3),
        metadata: { width: 1920, height: 1080, fps: 60, capturedAt: testTimestamp + 2, receivedAt: testTimestamp + 2, format: 'rgb' as const }
      }]
    ]);
  ...
  const overlayTestFrames = new Map([
    ['camera_a', {
      ...
      frameNumber: 2000,
      data: Buffer.alloc(1920 * 1080 * 3),
      metadata: { width: 1920, height: 1080, fps: 60, capturedAt: Date.now(), receivedAt: Date.now(), format: 'rgb' as const }
    }]
  ]);
